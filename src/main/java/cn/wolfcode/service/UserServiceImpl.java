...
//            return User.withUsername(admin).password("{noop}123456").authorities("admin", "ROLE_hr").build();
          return new LoginUser(username,"$2a$10$T9y5edISA2ni4a2e1e5EWe61d3NKopFKczJ8ugiRa2e3AcBo/LLfa", Arrays.asList("admin", "ROLE_admin","ROLE_hr"));
        } else if (roots.equals(username)) {
//            return User.withUsername(admin).password("{noop}123456").authorities("ROLE_hr", "dept").build();
            return new LoginUser(username,"$2a$10$rMFpvKv0cqzYP4AeYFhcjeDC9osocmLTChmi.daXLJ0WR3dS8Zmvy", Arrays.asList("ROLE_hr", "dept","admin"));
        }
